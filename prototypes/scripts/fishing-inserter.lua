...
local local_fishing_inserter_tick = function()
	if game.tick % 30 == 0 then
		for index, fishing_inserter in ipairs(global.coli.fishing_inserters) do
...
local local_fishing_inserter_added = function(ent)
	if ent.name == "fishing-inserter" then
		...
		table.insert(global.coli.fishing_inserters, ent)
	end
end
	...
	if entity.name == "fishing-inserter" then		
		for index, fishing_inserter in ipairs(global.coli.fishing_inserters) do
			if fishing_inserter.valid and fishing_inserter == entity then
				table.remove(global.coli.fishing_inserters, index)
				return
			end
		end
	end
...
function initFishingInserter()
	...
	if coli.ticks ~= nil then
		player_print("set up ok")

		--if settings.startup["coli-gone-fishing"].value == true then
			table.insert(coli.ticks,local_fishing_inserter_tick)
			table.insert(coli.on_added,local_fishing_inserter_added)
			table.insert(coli.on_remove,local_fishing_inserter_removed)
		--end
	end
end